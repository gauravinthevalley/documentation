...
On this pop-up window, configure the :guilabel:`Discount` percentage, select one of the following
discount options: :ref:`On All Order Lines <sales/all-order-lines>`, :guilabel:`Global Discount
<sales/global-discount>`, or :guilabel:`Fixed Amount <sales/fixed-amount>`.
...
Doing so removes the pop-up window, and returns to the sales order, which now shows that same
...
which has the cumulated value that is equivalent to the specified discount percentage (configured in
the :guilabel:`Discount` field of the pop-up window).
...
Doing so removes the pop-up window, and returns to the sales order, which now shows a new
...
.. important::
   Any product added (or removed) *after* the discount is added will not affect the discount value
   on the order line.
...
Doing so removes the pop-up window, and returns to the sales order, which now shows a
